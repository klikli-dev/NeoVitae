...
public interface SentientValueEffect {
    DeferredRegister<MapCodec<? extends SentientValueEffect>> VALUE_BASED_EFFECT_TYPE = DeferredRegister.create(NVRegistries.Keys.VALUE_BASED_EFFECT_TYPE, NeoVitae.MODID);
    Codec<SentientValueEffect> CODEC = Codec.lazyInitialized(() -> VALUE_BASED_EFFECT_TYPE
            .getRegistry()
            .get()
            .byNameCodec()
            .dispatch(SentientValueEffect::codec, Function.identity())
    );
    ...
    Supplier<MapCodec<ValueBasedExp>> VALUE_BASED_EXP = VALUE_BASED_EFFECT_TYPE.register("sentient_exp", () -> ValueBasedExp.CODEC);
    ...
    MapCodec<? extends SentientValueEffect> codec();
}
