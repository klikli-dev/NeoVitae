...
public class SentientHelper {
    public record UpgradeInstance(Holder<SentientUpgrade> upgrade, int level) {}
	...
	public static List<UpgradeInstance> getUpgrades(ItemStack stack) {
		...
		Object2FloatOpenHashMap<Holder<SentientUpgrade>> upgrades = stack.getOrDefault(NVDataComponents.UPGRADES, SentientStats.EMPTY).upgrades();

		for (Object2FloatMap.Entry<Holder<SentientUpgrade>> entry : upgrades.object2FloatEntrySet()) {
    ...
    public static void runIterationOnPlayer(Player player, BiConsumer<Holder<SentientUpgrade>, Integer> visitor) {
        runIterationOnItem(getChest(player), visitor);
    }

    public static final Object2FloatOpenHashMap<Holder<SentientUpgrade>> EMPTY_UPGRADE_MAP = new Object2FloatOpenHashMap<>();
    public static void runIterationOnItem(ItemStack stack, BiConsumer<Holder<SentientUpgrade>, Integer> visitor) {
        Object2FloatOpenHashMap<Holder<SentientUpgrade>> upgrades = stack.getOrDefault(NVDataComponents.UPGRADES, SentientStats.EMPTY).upgrades();
    ...
    public static int getLevelFromXp(Holder<SentientUpgrade> upgrade, float exp) {
    ...
    public static int nextLevelExp(Holder<SentientUpgrade> upgrade, float exp) {
    ...
    public interface UpgradeModifier {
        float apply(SentientUpgrade upgrade, int level, float currentValue);
    }
    ...
    private record ExpContext(
            Player wearer,
            Holder<SentientUpgrade> upgrade,
            float amount,
            boolean fromTome,
            ItemStack chest,
            Object2FloatOpenHashMap<Holder<SentientUpgrade>> upgrades,
    ...
    public static float applyExpToCap(Player wearer, Holder<SentientUpgrade> upgrade, float amount, boolean fromTome) {
        float rest = amount;
        float previous;
        do {
            previous = rest;
    ...
    public static float applyExp(Player wearer, Holder<SentientUpgrade> upgrade, float amount) {
        return applyExp(wearer, upgrade, amount, false);
    }

    public static float applyExp(Player wearer, Holder<SentientUpgrade> upgrade, float amount, boolean fromTome) {
    ...
    private static ExpContext gatherExpContext(Player wearer, Holder<SentientUpgrade> upgrade, float amount, boolean fromTome) {
        ItemStack chest = getChest(wearer);
        SentientArmourEvent.ExpGain event = NeoForge.EVENT_BUS.post(new SentientArmourEvent.ExpGain(wearer, upgrade, amount, fromTome));
        float eventAmount = event.getCurrentAmount();
        if (eventAmount <= 0) {
            return null;
        }

        Object2FloatOpenHashMap<Holder<SentientUpgrade>> upgrades = chest.getOrDefault(NVDataComponents.UPGRADES, SentientStats.EMPTY).upgrades().clone();
    ...
    private static void applyExpChange(ExpContext context, ExpResult result) {
        ...
        context.chest.set(NVDataComponents.UPGRADES, new SentientStats(context.upgrades));

        if (result.leveledUp) {
            context.chest.set(NVDataComponents.CURRENT_UPGRADE_POINTS, result.newTotalPoints);
            NeoForge.EVENT_BUS.post(new SentientArmourEvent.LevelUp(context.wearer, context.upgrade, result.oldLevel, result.newLevel));
            context.wearer.displayClientMessage(Component.translatable("chat.neovitae.sentient_upgrade.level_up", Component.translatable(SentientUpgrade.descriptionId(context.upgrade.getKey())), result.newLevel), true);
        }
    }
    
    public static Component getTooltip(Holder<SentientUpgrade> upgrade, float exp, boolean hasShiftDown) {
        ...
        if (upgrade.is(NVTags.Sentient.IS_DOWNGRADE)) {
        ...
        MutableComponent mutable = Component.translatable(SentientUpgrade.descriptionId(upgrade.getKey())).withStyle(style, colour);
    ...
    public static Object2FloatOpenHashMap<Holder<SentientUpgrade>> fromHolderSet(HolderSet<SentientUpgrade> template) {
        return fromHolderSet(template, 1);
    }

    public static Object2FloatOpenHashMap<Holder<SentientUpgrade>> fromHolderSet(HolderSet<SentientUpgrade> template, float val) {
        Object2FloatOpenHashMap<Holder<SentientUpgrade>> ret = new Object2FloatOpenHashMap<>();
    ...
    public static int recalcPoints(Player player) {
        ItemStack chest = getChest(player);
        Object2FloatOpenHashMap<Holder<SentientUpgrade>> upgrades = chest.getOrDefault(NVDataComponents.UPGRADES, SentientStats.EMPTY).upgrades();

        int total = 0;
        for (Map.Entry<Holder<SentientUpgrade>, Float> entry : upgrades.object2FloatEntrySet()) {
    ...
    public static int getMaxLevel(Holder<SentientUpgrade> upgrade) {
